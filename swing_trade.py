...
策略（十六個，原有十個 + 新增六個）：
  原有策略：
    ...
    E多 強勢連漲       ES空 弱勢連跌
  新增策略（均量類）：
    F多 均量放大（近5日均量 > 20日均量 × 1.2 且今日爆量上漲）
    FS空 均量萎縮（近5日均量 < 20日均量 × 0.8 且今日爆量下跌）
    G多 量能潮汐（連續3日縮量後今日放量上漲）
    GS空 量能枯竭（連續3日縮量後今日放量下跌）
  新增策略（K棒型態類）：
    H多 鎚子K（下影線 ≥ 2×實體，前日下跌）
    HS空 射擊之星（上影線 ≥ 2×實體，前日上漲）
    I多 吞噬多（大陽線吞噬前日陰線）
    IS空 吞噬空（大陰線吞噬前日陽線）
...
def fetch_data_sinopac(codes: list, days: int,
    ...
    for idx, code in enumerate(codes, 1):
        ...
        try:
            ...
            df_day["Volume"] = df_day["Vol_K"] * 1000
...
def _precompute_signals_vec(sub: pd.DataFrame,
    ...
    # ── 組合成訊號陣列（shape: n × 18，欄位順序 = _STRAT_KEYS）──
    def _f(cond):
